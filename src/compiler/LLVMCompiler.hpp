...
public:
	...
	value insn_call(Type return_type, std::vector<value> args, R(*func)(A...), std::string name = "", bool exception = false) const {
		return insn_call(return_type, args, (void*) func, std::move(name), exception);
	}
	value insn_call(Type return_type, std::vector<value> args, void* func, std::string name = "", bool exception = false) const;
