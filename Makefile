...
travis:
	...
	docker run -e COVERALLS_REPO_TOKEN="`printenv COVERALLS_REPO_TOKEN`" leekscript /bin/bash -c "printenv; cd leekscript; make -j4 && make test && cpp-coveralls --gcov-options='-r'"
